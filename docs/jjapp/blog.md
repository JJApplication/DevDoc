...
现在Blog升级到`NEXT`版本， 你可以体验更棒的Blog服务
...
Blog是一个个人博客项目，基于`markdown`依赖于强大的`marked`实现文档的实时渲染
...
`br_ng`分支提供了静态文件的直接代理

`br_next`分支为NEXT设计的分支
...
`APP_IMAGE_ROOT` 上传图片存储的目录默认为`./images`
...
> 在BlogNEXT版本中因为使用`vite`编译项目，html中的环境变量加载方式不再受支持
>
> 但是你仍然可以使用`import.meta.env`来使用`.env`中配置的环境变量
...
所有的图片资源都存放在`~/src/assets`目录下

`avatar.jpg` 为博客头像

`background.jpg/webp` 为首页的背景图
